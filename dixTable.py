...
import sys, os, argparse, itertools, re, subprocess, urllib.request, logging, collections, textwrap, json
...
def updateOrCreatePages(location, pairName, updateStats, editToken):
    ...
    if not statusPage:
        ...
        if statsPage:
            ...
            pageContents = """
            {{{{TOCD}}}}
            This is a language pair translating between [[{0}]] and [[{1}]]. The pair is currently located in [https://github.com/apertium/{3} {2}].
    ...
    for package in gitData:
        ...
        if packageMatch:
            ...
            if {srcLang, targetLang} <= languages:
                ...
                secondaryPairs[frozenset({srcLang, targetLang})] = (dirPath, packageName)
